...
- `IStorage` interface — `Ping`, `GetState`/`SetState`/`SetPts`/`SetQts`/`SetDate`/`SetSeq`/`SetDateSeq`, `SetChannelPts`/`GetChannelPts`/`ForEachChannels`, `SetChannelAccessHash`/`GetChannelAccessHash`, `AccessHashSaveHandler`, `SetUserAccessHash`/`GetUserAccessHash`, `TelegramStorageCompatible() updates.StateStorage`, `TelegramAccessHasherCompatible() updates.ChannelAccessHasher`.
...
- `ISessionStorage` interface — `LoadSession`, `StoreSession`, `TelegramSessionStorageCompatible()`.
- `SessionStorage` struct + `NewSessionStorage(entityID, secret) *SessionStorage` (in-memory repo) / `NewSessionStorageWithCustomRepo(entityID, secret, repo IEntitySessionStorageRepo) *SessionStorage`.
- `IEntitySessionStorageRepo` interface — `UpdateAuthKey`, `FetchAuthKey`.
