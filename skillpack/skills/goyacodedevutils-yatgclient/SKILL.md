...
- `Client` struct (embeds `*telegram.Client`) + `ClientOptions{ AppID, AppHash, EntityID, TelegramOptions, ChunkSize, BackgroundConnectConfig }`; `NewClient(options, log) *Client`.
...
- `NewUpdateManagerWithYaStorage(entityID, handler, storage yatgstorage.IStorage) *updates.Manager`.
